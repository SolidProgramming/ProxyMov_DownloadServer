    ...
    : IJob
{
    ...
    public static async Task<bool> InitAsync(WebProxy? proxy = null)
    {
        ...
        using var noProxyHttpClient = HttpClientFactory.CreateHttpClient();

        var (success, ipv4) = await HttpClient.GetIPv4();
        var (noProxySuccess, NoProxyIpv4) = await noProxyHttpClient.GetIPv4();
    ...
    public async Task CheckForNewDownloads()
    {
        ...
        while (DownloadQue!.Count != 0)
        {
            ...
            var episodeDownload = DownloadQue.Dequeue();
            ...
            var originalEpisodeName = episodeDownload.Download.Name;
            ...
            var languageRedirectLinks = HosterHelper.GetLanguageRedirectLinks(html);
            ...
            var episodeLanguages =
                episodeDownload.Download.LanguageFlag.GetFlags<Language>(Language.None);
            var redirectLanguages =
                languageRedirectLinks.Keys.Where(lang => episodeLanguages.Contains(lang));

            var downloadLanguages = episodeLanguages.Intersect(redirectLanguages);

            var finishedDownloadsCount = 1;

            foreach (var language in downloadLanguages)
            {
                ...
                var m3u8Url = await GetEpisodeM3U8(url, downloaderPreferences);
                ...
                var result = await converterService.StartDownload(m3u8Url, episodeDownload.Download,
    ...
    public static void RemoveHandlers()
    {
        if (CronJobEvent is not null)
            foreach (var d in CronJobEvent.GetInvocationList())
                CronJobEvent -= (CronJobEventHandler)d;

        if (CronJobErrorEvent is not null)
            foreach (var d in CronJobErrorEvent.GetInvocationList())
                CronJobErrorEvent -= (CronJobErrorEventHandler)d;

        if (CronJobDownloadsEvent is not null)
            foreach (var d in CronJobDownloadsEvent.GetInvocationList())
    ...
    private async Task<string?> GetEpisodeM3U8(string streamUrl, DownloaderPreferencesModel downloaderPreferences)
    {
        ...
        var proxyLogText = $"| Url: {downloaderPreferences.ProxyUri}@{downloaderPreferences.ProxyUsername}";
        ...
        try
        {
            var videoPageHtml = await GetVideoPageHtml(page, streamUrl);

            if (string.IsNullOrEmpty(videoPageHtml)) return null;

            if (TryGetVideoSource(videoPageHtml, out var m3u8)) return m3u8;

            var m3u8FromJwPlayer = await GetM3U8ViaJwPlayer(page);
    ...
    private static bool TryGetVideoSource(string html, out string? m3u8)
    {
        ...
        var m3u8NodeMatch = regex.Match(html);
        ...
        var hlsMatch = new Regex("'hls': '(.*?)',").Match(html);
        ...
        var playerSourceMatch = new Regex("<source src=\"(.*?)\"").Match(html);
    ...
    {
        var removeSuccess = await apiService.RemoveFinishedDownload(episodeDownload);
