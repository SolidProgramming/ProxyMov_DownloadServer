    ...
    : IConverterService
{
        ...
        DownloaderPreferencesModel downloaderPreferences, ConverterSettingsModel? converterSettings)
    {
        ...
        var filePath = GetFileName(download, downloadPath, converterSettings);

        var streamDuration = await GetStreamDuration(streamUrl, downloaderPreferences);
        ...
        {
            var durationExistingFile = TimeSpan.Zero;
        ...
        {
            var proxyUri = downloaderPreferences.ProxyUri.Replace("http://", "").Replace("https://", "");
            proxyAuthArgs =
                $"-http_proxy http://{downloaderPreferences.ProxyUsername}:{downloaderPreferences.ProxyPassword}@{proxyUri}";
        }

        var hwAccell = "-hwaccel cuvid -hwaccel_output_format cuda";
        var useHwAccell = false;
        ...
        var binPath = Helper.GetFFMPEGPath();
        ...
        ConverterSettingsModel? converterSettings)
    {
        ...
        var filePath = $"{download.Name}_{seasonFolderName}{episodeFolderName}.ts";

        if (converterSettings is not null && converterSettings.FileFormat != FileFormat.ORIGINAL)
        {
            var filePathWithExtension = Path.ChangeExtension(filePath, converterSettings.FileFormat.ToFileFormat());
    ...
    private static void ReadOutput(string output)
    {
        ...
        try
        {
            var bitrateMatch = Regex.Match(output, "bitrate=\\s*(\\d+\\.?\\d*)kbits/s");

            if (!bitrateMatch.Success) return;

            var bitrateText = bitrateMatch.Groups[1].Value;
            progress.Bitrate = double.Parse(bitrateText);

            var sizeMatch = Regex.Match(output, "size=\\s*(\\d+)Ki?B time", RegexOptions.IgnoreCase);

            if (!sizeMatch.Success) return;

            var sizeText = sizeMatch.Groups[1].Value;
            progress.Size = long.Parse(sizeText);

            var timeMatch = Regex.Match(output, "time=([\\d:]+)");

            if (!timeMatch.Success) return;

            var timeText = timeMatch.Groups[1].Value;
            progress.Time = TimeSpan.Parse(timeText);

            var bytesDownloaded = progress.Size * 1024 - PreviousDownloadSize;
            ...
            var progressPercent = 100.0d * (progress.Time.TotalSeconds / Download!.StreamDuration.TotalSeconds);

            if (progressPercent <= 0.0) return;

            progress.ProgressPercent = Convert.ToInt32(progressPercent);

            var speedMatch = Regex.Match(output, "speed=.*?(\\d+(?:[\\.\\,]\\d+)*)");

            if (!speedMatch.Success) return;

            var speedText = speedMatch.Groups[1].Value;
            progress.EncodingSpeed = float.Parse(speedText);

            var fpsMatch = Regex.Match(output, "fps=.*?(\\d+(?:[\\.\\,]\\d+)*)");

            if (!fpsMatch.Success) return;

            var fpsText = fpsMatch.Groups[1].Value;
    ...
    {
        var elapsedTime = DateTime.Now.Subtract(downloadStartTime);

        var remainingPercentage = 100 - completedPercentage;
        var estimatedTotalTime = elapsedTime.TotalSeconds / completedPercentage * 100;
        var remainingTime = estimatedTotalTime * remainingPercentage / 100;
        var remainingTimeSpan = TimeSpan.FromSeconds(remainingTime);
    ...
    private async Task<TimeSpan> GetStreamDuration(string streamUrl, DownloaderPreferencesModel downloaderPreferences)
    {
        ...
        for (var attempt = 1; attempt < MaxGetStreamDurationRetries; attempt++)
        {
    ...
    private async Task<TimeSpan> TryGetStreamDuration(string ffProbeArgs, int attemptCount)
    {
        ...
        if (TimeSpan.TryParse(stdOut, out var duration)) return duration;
    ...
    private static async Task<TimeSpan> GetStreamDurationFromFile(string filePath)
    {
        ...
        var binPath = Helper.GetFFProbePath();
        ...
        var ffProbeToken = ffProbeCTS.Token;
        ...
        var output = StdOutBuffer.ToString().TrimEnd();
